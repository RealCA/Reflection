...
void IMaterialGraph::CreatedFallbackGraphNotification() const {
	FNotificationInfo Info = FNotificationInfo(FText::FromString("Created Fallback Graph for " + GetAssetName()));
	Info.ExpireDuration = 7.0f;
	Info.bUseLargeFont = true;
	Info.bUseSuccessFailIcons = true;
	Info.WidthOverride = FOptionalSize(350);
