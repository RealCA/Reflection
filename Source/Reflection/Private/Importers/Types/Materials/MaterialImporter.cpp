...
#include "Importers/Types/Materials/MaterialFallback.h"
...
bool IMaterialImporter::Import() {
	...
	bool bFallbackGraphPending = false;
	...
	if (ExpressionContainer->Num() == 0) {
		...
		else if (GetSettings()->AssetSettings.Material.FallbackGraph) {
			bFallbackGraphPending = true;
		}
		else {
#endif
...
#if ENGINE_UE5
	/* Graph-less imports: synthesize a fallback expression graph from the
	 * compiled output, so parameters stay editable even without a preprocessed batch.
	 * Pass the full export object: LoadedMaterialResources/CachedExpressionData/
	 * ReferencedTextures live at the export top level, not inside Properties. */
	if (bFallbackGraphPending) {
		if (Reflection::MaterialFallback::CreateFallbackGraph(this, AssetExport->GetJsonObject().JsonObject)) {
			CreatedFallbackGraphNotification();
			GLog->Log(*FString::Printf(TEXT("Reflection: Material '%s' had no editor Expressions; generated approximate fallback graph from compiled output - review manually"), *GetAssetName()));
		}
		else {
			SpawnMaterialDataMissingNotification();
		}
