...
void UPropertySerializer::DeserializePropertyValue(FProperty* Property, const TSharedRef<FJsonValue>& JsonValue, void* OutValue, UObject* OptionalOuter) {
	const FJsonDebug::FScopedJsonDebugContext JsonCtx(FString::Printf(
		TEXT("DeserializePropertyValue %s (%s) on %s"),
		*Property->GetName(), *Property->GetClass()->GetName(),
		OptionalOuter ? *OptionalOuter->GetName() : TEXT("<none>")));
	...
	if (MapProperty) {
		...
		if (StructProperty->Struct == FGameplayTag::StaticStruct()) {
			/* A Number/String here is malformed export data; AsObject() would hand back an empty
			 * object and dereferencing it crashes. Leave the default tag instead. */
			if (NewJsonValue->Type != EJson::Object) {
				return;
			}
		 ...
		 * A Number/String here is malformed export data: AsObject() would hand back an empty object whose ToSharedRef()
		 * asserts. Leave the struct at its default value instead of crashing the editor. */
		if (NewJsonValue->Type == EJson::Object) {
		...
		if (JsonValue->Type == EJson::String) {
			...
			if (ByteProperty->Enum == nullptr) {
				UE_LOG(LogReflection, Warning, TEXT("Enum property '%s' has no resolved enum, falling back to 0 for value '%s'"),
					*Property->GetName(), *EnumAsString);

				ByteProperty->SetIntPropertyValue(OutValue, int64(0));
				return;
			}

			int64 EnumerationValue = 0;
			bool bCanResolve = true;

			/* A UserDefinedEnum shell can still be unpopulated (empty, default ECppForm::Regular);
			 * resolving a namespaced value would make the engine assert in GenerateFullEnumName. */
			if (const UUserDefinedEnum* UserDefinedEnum = Cast<UUserDefinedEnum>(ByteProperty->Enum)) {
				if (UserDefinedEnum->GetCppForm() != UEnum::ECppForm::Namespaced || UserDefinedEnum->NumEnums() == 0) {
					UE_LOG(LogReflection, Warning, TEXT("Unpopulated UserDefinedEnum '%s' for property '%s', falling back to 0 for value '%s'"),
						*ByteProperty->Enum->GetName(), *Property->GetName(), *EnumAsString);
			...
			if (bCanResolve) {
				EnumerationValue = ByteProperty->Enum->GetValueByNameString(EnumAsString);
			}
		...
		*static_cast<FString*>(OutValue) = StringValue;
		...
		if (Enum == nullptr) {
			UE_LOG(LogReflection, Warning, TEXT("Enum property '%s' has no resolved enum, skipping value '%s'"),
				*Property->GetName(), *EnumAsString);
		...
		/* A UserDefinedEnum shell can still be unpopulated (empty, default ECppForm::Regular);
		 * resolving a namespaced value would make the engine assert in GenerateFullEnumName. */
		if (const UUserDefinedEnum* UserDefinedEnum = Cast<UUserDefinedEnum>(Enum)) {
			if (UserDefinedEnum->GetCppForm() != UEnum::ECppForm::Namespaced || UserDefinedEnum->NumEnums() == 0) {
				UE_LOG(LogReflection, Warning, TEXT("Unpopulated UserDefinedEnum '%s' for property '%s', skipping value '%s'"),
					*Enum->GetName(), *Property->GetName(), *EnumAsString);
		...
		int64 EnumerationValue = Enum->GetValueByNameString(EnumAsString);
