...
public:
	 ...
	 * subobjects - from the main export and a compile, so a container that holds one imports only
	 * that export. ControlRigs export under RigVMBlueprintGeneratedClass, so they have to be in the
	 * list too - otherwise shell creation can't locate their class export or pick the importer. */
	FString GetBlueprintType() const {
		static const TArray<FString> BlueprintTypes = {
